...
class CriteriaService {
    ...
    getPaginatedCriteria({ page = 1, limit = 300, search, dimension } = {}) {
        return criteriaRepo.getPaginatedCriteria({ page, limit, search, dimension });
    }
    ...
    getCriterionByName(normalizedName) {
        return criteriaRepo.getCriterionByName(normalizedName);
    }
    ...
    getCriteriaForEntity(entityId) {
        return criteriaRepo.getCriteriaForEntity(entityId);
    }
    ...
    getCriterionAssociations(criterionId) {
        return {
            sources: criteriaRepo.getAssociatedSources(criterionId),
            targets: criteriaRepo.getAssociatedTargets(criterionId)
        };
    }
     ...
     */
    deleteCriterion(id) {
        return criteriaRepo.deleteCriterion(id);
    }
    ...
    getSimilarCriteria(criterionId, limit = 10) {
        const target = criteriaRepo.getCriterionById(criterionId);
        if (!target) throw new Error('Criterion not found');

        const all = criteriaRepo._getAllCriteriaWithEmbeddings();
        const similarities = [];
    ...
    mergeCriteria(keepId, removeId) {
        const removeCriterion = criteriaRepo.getCriterionById(removeId);
        if (!removeCriterion) {
            throw new Error('Criterion to remove not found');
        }
        criteriaRepo.mergeCriteria(keepId, removeId, removeCriterion.displayName);
    }
