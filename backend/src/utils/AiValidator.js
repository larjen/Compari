/**
 * @module AiValidator
 * @description Validation utilities for AI extraction workflows.
 * 
 * @responsibility
 ...
 * logging (input validation failures) that doesn't need file persistence.
 */
 ...
 * 
 * @example
 * if (!AiValidator.validateInputText(jobDescription, 'Job posting')) {
 *     return;
 * }
 */
 ...
 *     logService.logTerminal('WARN', 'WARNING', 'Workflow', 'AI returned empty dimensions', ...);
 *     return;
 * }
 */
function isEmptyExtraction(dimensions, activeDimensions) {
    if (!dimensions) {
        return true;
    }
